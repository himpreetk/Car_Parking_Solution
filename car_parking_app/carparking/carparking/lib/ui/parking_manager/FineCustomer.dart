...
class FineCustomer extends StatefulWidget {
  String header_title = "";
  String userEmail = "";
  String username = "";
  String user_contact, user_location;

  FineCustomer(this.header_title, this.userEmail, this.username, this.user_contact, this.user_location);

  @override
  _FineCustomerState createState() =>
      _FineCustomerState(header_title, userEmail, username, this.user_contact, this.user_location);
}

class _FineCustomerState extends State<FineCustomer> {
  ...
  _FineCustomerState(this.appBartitle, this.userEmail, this.username, this.user_contact, this.user_location);
...
//      height: 150,
  ...
  saveAccountInfo() async {
    ...
    int newCredit =  int.parse(credit) - int.parse(fine);
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      content: Text(fine.toString()+ " Amount Fine charged"),
    ));
  ...
  getCustomers() async{
    QuerySnapshot activeusersSnapshot = await FirebaseDBHelper().getActiveUserByGroup("Customer");
    for (int i = 0; i < activeusersSnapshot.docs.length; i++) {
      if (activeusersSnapshot.docs[i].data()["location"] == user_location) {
        usersList.add(new UsersModel(
          activeusersSnapshot.docs[i].data()["user_email"],
          activeusersSnapshot.docs[i].data()["user_group"],
          activeusersSnapshot.docs[i].data()["user_name"],
          activeusersSnapshot.docs[i].data()["user_contact"],
          activeusersSnapshot.docs[i].data()["location"],
          activeusersSnapshot.docs[i].data()["active_status"],
          activeusersSnapshot.docs[i].data()["vehicle_no"],
        ));
      }
    }
